...
RunResult Run(const fs::path &path, uint32_t resolution, uint32_t max_iterations) {
    ...
    if (reference.Values.size() != SolvedModeCount || preferred.Eigenvalues.size() != SolvedModeCount)
        throw std::runtime_error("oracle or preferred route did not converge");
    ...
    const double spectrum = (preferred.Eigenvalues.segment(6, AcceptedModeCount - 6) - reference.Values.segment(6, AcceptedModeCount - 6)).norm() /
        reference.Values.segment(6, AcceptedModeCount - 6).norm();
    ...
    const auto shapes = finite_cell_benchmark::CompareSameDiscretizationModeShapes(
        operation, reference.Values, reference.Vectors, preferred.Eigenvectors, 6, AcceptedModeCount
    );
    const auto fractions = PhysicalCellFractions(operation);
    std::println(
        "real-mesh object={} triangles={} grid={}x{}x{} dofs={} active={} interior={} cut={} fill={:.3f} cell_fraction={:.2e}/{:.3f} slivers={} iterations={} fallback={}/{}/{:.3e} spectrum={:.3e} residual={:.3e} orthogonality={:.3e} reference={:.3e}/{:.3e} cluster_mac={:.6f}",
        ...
        preferred_certification.MassOrthogonalityError, reference.RelativeResidual,
