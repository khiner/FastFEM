...
double Dot(VectorView<const double> a, VectorView<const double> b) {
...
double Norm(VectorView<const double> vector) {
    return cblas_dnrm2(LapackSize(vector.Count), vector.Values, LapackSize(vector.Stride));
}

double Norm(MatrixView<const double> matrix) {
    double squared{};
...
void Scale(double scale, VectorView<double> vector) {
    cblas_dscal(LapackSize(vector.Count), scale, vector.Values, LapackSize(vector.Stride));
}

void Scale(double scale, MatrixView<double> matrix) {
    for (size_t column = 0; column < matrix.Columns; ++column)
        cblas_dscal(LapackSize(matrix.Rows), scale, matrix.Values + column * matrix.LeadingDimension, 1);
}

void AddScaled(double scale, VectorView<const double> source, VectorView<double> destination) {
...
void AddScaled(double scale, MatrixView<const double> source, MatrixView<double> destination) {
...
Matrix<double> Multiply(MatrixView<const double> a, MatrixView<const double> b) {
    Matrix<double> result(a.Rows, b.Columns);
    Multiply(a, b, result.View());
    return result;
}

Matrix<double> TransposeMultiply(MatrixView<const double> a, MatrixView<const double> b) {
    Matrix<double> result(a.Columns, b.Columns);
    TransposeMultiply(a, b, result.View());
    return result;
}

void SubtractProduct(MatrixView<double> destination, MatrixView<const double> a, MatrixView<const double> b) {
    Multiply(a, b, destination, -1, 1);
}

void ScaleColumns(MatrixView<double> matrix, VectorView<const double> scales) {
...
void ScaleRowsAndColumns(MatrixView<double> matrix, VectorView<const double> scales) {
...
void Symmetrize(MatrixView<double> matrix) {
...
Matrix<double> ColumnScaledDifference(
...
double Maximum(VectorView<const double> vector) {
...
double MaximumAbsolute(VectorView<const double> vector) {
    double result{};
