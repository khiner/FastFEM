    ...
    for (uint32_t sample = 0; sample < stencils.size(); ++sample)
        for (uint32_t node = 0; node < stencils[sample].Count; ++node)
            for (uint32_t component = 0; component < 3; ++component)
                for (size_t mode = first_mode; mode < modes.cols(); ++mode)
...
inline ModeShapeComparison CompareSameDiscretizationModeShapes(
    ...
    result.PairedMacMinimum = std::numeric_limits<double>::infinity();
    for (size_t mode = 0; mode < accepted_count; ++mode)
        result.PairedMacMinimum = std::min(result.PairedMacMinimum, std::pow(overlap(mode, mode), 2));
    result.BestMacMinimum = std::numeric_limits<double>::infinity();
    for (size_t mode = 0; mode < accepted_count; ++mode) {
        double maximum{};
        for (size_t column = 0; column < overlap.cols(); ++column) maximum = std::max(maximum, std::pow(overlap(mode, column), 2));
        result.BestMacMinimum = std::min(result.BestMacMinimum, maximum);
    }
    result.ClusterMacMinimum = 1;
    for (size_t first = first_mode; first < accepted_end;) {
        size_t end = first + 1;
